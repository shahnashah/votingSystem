Let me continue with the Post Form Modal and then implement the remaining components for the committee dashboard:
...
Now, let's implement the Nomination Management section, which will allow committee members to approve/reject nominations:
...
const NominationManagement = () => {
    ...
    const fetchElections = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/committee/elections`,
          {
            headers: {
  ...
  const fetchNominations = async (electionId, status) => {
    try {
      setLoading(true);
      const url = `${API_BASE_URL}/committee/elections/${electionId}/nominations${
  ...
  const handleApproveNomination = async (id) => {
    try {
      await axios.put(
        `${API_BASE_URL}/committee/nominations/${id}/approve`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleRejectNomination = async (id, rejectionReason) => {
    try {
      await axios.put(
        `${API_BASE_URL}/committee/nominations/${id}/reject`,
        { rejectionReason },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
...
Now, let's implement the Link Management section to generate and manage registration, nomination, and voting links:

```jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { Link, Copy, RefreshCw, Calendar, Share2, LinkIcon } from 'lucide-react';
    ...
    try {
      ...
      const updatedElections = elections.map(election =>
        election._id === currentElection._id
          ? { ...election, [`${linkType}Link`]: response.data[`${linkType}Link`] }
          : election
      );
        ...
        <>
          <div className="mb-6">
            ...
            <select
              ...
              onChange={(e) => {
                const selected = elections.find(election => election._id === e.target.value);
          ...
          {currentElection && (
            <div className="bg-white rounded-lg shadow overflow-hidden p-6">
              <div className="mb-6">
                <h2 className="text-lg font-semibold mb-2">{currentElection.title}</h2>
                <div className="flex items-center text-sm text-gray-500">
                  <Calendar size={16} className="mr-1" />
                  <span>
                    Voting period: {formatDate(currentElection.votingStart)} to {formatDate(currentElection.votingEnd)}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <LinkCard
                  ...
                  link={currentElection.registrationLink ? getFullLink(currentElection.registrationLink) : 'Not generated yet'}
                  onRegenerate={() => handleRegenerate('registration')}
                  onCopy={() => copyToClipboard(getFullLink(currentElection.registrationLink), 'registration')}
                ...
                <LinkCard
                  ...
                  link={currentElection.nominationLink ? getFullLink(currentElection.nominationLink) : 'Not generated yet'}
                  onRegenerate={() => handleRegenerate('nomination')}
                  onCopy={() => copyToClipboard(getFullLink(currentElection.nominationLink), 'nomination')}
                ...
                <LinkCard
                  ...
                  link={currentElection.votingLink ? getFullLink(currentElection.votingLink) : 'Not generated yet'}
                  onRegenerate={() => handleRegenerate('voting')}
                  onCopy={() => copyToClipboard(getFullLink(currentElection.votingLink), 'voting')}
              ...
              <div className="mt-8 p-4 bg-yellow-50 rounded-md">
                ...
                <ul className="mt-2 text-sm text-yellow-700 list-disc pl-5 space-y-1">
                  <li>Regenerating a link will invalidate the previous link.</li>
                  <li>Make sure to distribute the new links to all participants.</li>
                  <li>Voting links will only work during the set voting period.</li>
                </ul>
              </div>
            </div>
          )}
